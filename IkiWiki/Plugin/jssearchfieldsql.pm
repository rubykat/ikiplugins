...
package IkiWiki::Plugin::jssearchfieldsql;
...
sub set_up_search {
	...
	if (!exists $params{$p})
	{
	    error gettext("jssearchfieldsql: missing $p parameter");
	}
    }
    if (!exists $config{sqlreport_databases}->{$params{database}})
    {
	error(gettext(sprintf('jssearchfieldsql: database %s does not exist',
		$params{database})));
    }
    my $table=$params{table};
    my $where=$params{where};
    my $dbfile=$config{sqlreport_databases}->{$params{database}};
    ...
    if (!$tot_sth)
    {
	error("Can't prepare query $total_query: $DBI::errstr");
	return 0;
    }
    my $rv = $tot_sth->execute();
    if (!$rv)
    {
	error("Can't execute query $total_query: $DBI::errstr");
	return 0;
    }
    my $total = 0;
    my @trow;
    while (@trow = $tot_sth->fetchrow_array)
    {
	$total = $trow[0];
    }
    if ($total == 0)
...
EOT
    ...
    #
    # The array of records; look them up in the database
    #
    my $query = sprintf('SELECT page, %s FROM %s WHERE %s',
	join(', ', @fields),
	$params{table},
	$params{where});
    ...
    if (!$sth)
    {
	error("Can't prepare query $query: $DBI::errstr");
	return 0;
    }
    $rv = $sth->execute();
    if (!$rv)
    {
	error("Can't execute query $query: $DBI::errstr");
	return 0;
    }
